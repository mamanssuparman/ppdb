...
class Umum extends CI_Controller {
	...
	public function save()
	{
		...
		if ($pencariannisn->num_rows()>0) {
			...
			$this->Mdl_Adminn->insert_pendaftar_umum($namasimpanakta,$namasimpankk,$namasimpanijazah,$namasimpanskhun);
			$this->db->trans_complete();	
		}
		$uri2=md5($this->input->post('nisn',TRUE));
		redirect('Umum/Cetak/'.$this->input->post('no_peserta_un',TRUE).'/'.$uri2,'refresh');
	}
